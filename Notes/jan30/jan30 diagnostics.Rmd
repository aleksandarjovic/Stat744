...
#(ggplot(a0,aes(sample=value))+      #STAT QQ and STAT QQLINE NOT WORKING
 #   stat_qq()+
  #  facet_wrap(~type)+
...
* LOESS does a good job with the mean values (what this grouping was trying to accomplish)
...
ggplot(a3,aes(age,.resid,colour=urban))+
    geom_point()+
    geom_smooth(method="gam",formula =y ~ s(x, k=25)) + #here choosing max number of knots
    facet_wrap(~livch)
```

Otherwise for splines, could use method="lm", formula= y~ns(x,5)
...
Similar to quantile quantile plots. Good way to evaluate a statistical test.
Simulating p values, in the null case, we ought to get a uniform distribution. 
...
Recall Wald's Test Statistic.
Should get a straight line once doign this transformation, if it's straight, it's quadratic. (harder to tell if quadratic by looking at the curved line (top graphs))
...
sumfun <- function(x)sum(x==0) #summary function counts 0s, each column seperate simulation, each row a separate observation
